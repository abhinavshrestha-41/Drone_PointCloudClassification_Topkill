...
# **MAKE SURE THIS FILE IS IN WORKING DIRECTORY**
source("damageAssessSummary_function.R") # if file not present, uncomment code near the end of THIS script to create function.
...
CSV_exportDir = file.path(exportPATH, "damageAssessmentSummaries_CSVs")

# INPUT

shpFile1_Name <- "M2_subset_TreeDamageAlgo_TreeSegPolygons.shp"
# shpFile2_Name <- "{ShapefileNAME}.shp" # if there are multiple sites

# OUTPUT

missionSiteSummary_csvName = "M2_subset_li2012_damageTypeSummaryTable.csv"

# -------------------------------------------------------------------------



shpFile1_Path <- file.path(exportPATH, shpFile1_Name)
#shpFile2_Path <- file.path(exportPATH, shpFile2_Name)

shpFile1_obj <- terra::vect(shpFile1_Path)
#shpFile2_obj <- terra::vect(shpFile2_Path)


damageAssessSummary(shpFile1_obj)
...
# Summary for mission site:
write.csv(Damage_Assessment_OutputTable, file.path(CSV_exportDir, missionSiteSummary_csvName), row.names = FALSE)


# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

# If dammageAssessSumary() function source file is not available, please un-comment the following code from the comment "UNCOMMENT FROM HERE TO RUN FUNCTION:" and run to create the function in the currently running R-environment.
...
#-----------------------------------------------------------------------------------------#
# A Shrestha - UIdaho - July 16 2023 - NOTE only use for 'SpatVector' objects pkg:`terra` #
#-----------------------------------------------------------------------------------------#
...
# UNCOMMENT FROM HERE TO RUN FUNCTION:
